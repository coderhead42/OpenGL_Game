...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'm':
            fgets(buf, sizeof(buf), file);
            sscanf(buf, "%s %s", buf, buf);
            model->materials[nummaterials].texture_file = strdup(buf);
            break;
        default:
            /* eat up rest of line */
            fgets(buf, sizeof(buf), file);
 ...
 */
GLvoid
glmDraw(GLMmodel* model, GLuint mode, bool flag)
{
    static GLuint i;
	...
	if(group->angle!=0){
       ...
       if (mode & GLM_MATERIAL) {
        ...
        if(flag && material->texture_file!=NULL){
            //char tname[50];
            //strcpy(tname,material->texture_file);
